package com.bwie.weidushopping.homepage.fragmentshouye.adapter.keysearchselect;
 ...
 * date:2018/12/7
 * author:张自力(DELL)
 * function:  首页Fragment  用户根据关键字keyword进行查询商品的Adapter
 */

public class KeySearchSelectAdapte extends XRecyclerView.Adapter<KeySearchSelectAdapte.ViewHolder>{
    //定义一个接口
    public interface KeySearchOnClickListener{
       void onChanger(int id);
    }
    private KeySearchOnClickListener mlistener;
    public void setKeySearchOnClickListener (KeySearchOnClickListener listener){
        this.mlistener = listener;
    }

    private Context mContext;
    private List<KeySeacherBean.ResultBean> mList;
    public KeySearchSelectAdapte(Context context, List<KeySeacherBean.ResultBean> list) {
        mContext = context;
        mList = list;
    }
